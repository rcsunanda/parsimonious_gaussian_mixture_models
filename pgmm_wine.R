...
wine_clust<-pgmmEM(x, rG=1:5, rq=1:5, zstart=1, loop=3, modelSubset=models_to_run)

# Take the max BIC value among the different PGMMs for each (G,q) pair
bic_array = simplify2array(wine_clust$bic) # Convert to 3D array dim = [5, 5, 6] (rows=G, cols=q, depth=PGGM models)
...
library(EMCluster)
RRand(wine$Type, wine_clust$map)

# Fit Standard GMM (mclust ) model
mod1 <- Mclust(x, G=1:5, modelNames='VVI')
summary(mod1, parameters = FALSE)

table(wine[,1], mod1$classification)

RRand(wine$Type, mod1$classification)
